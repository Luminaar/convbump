...
SCHEMA_MAPPING: DefaultDict[ProjectTypeEnum, str] = DefaultDict(lambda: "-TYPE.NUMBER")
...
class PreRelease:
    ...
    @classmethod
    def parse(cls, value: str, *, project_type: ProjectTypeEnum) -> "PreRelease":
        schema = SCHEMA_MAPPING[project_type]

        maybe_parsed = parse_version(schema, SCHEMA_PARTS_PARSING, value)
        if maybe_parsed:
            with suppress(KeyError):
                return cls.from_parsed_dict(maybe_parsed, project_type=project_type)

        raise ValueError(
            "Invalid pre-release value, which do not match any supported " "project type"
        )
    ...
    def update(self, config: UpdateConfig) -> Optional["PreRelease"]:
        ...
        if config.is_breaking_change:
            ...
            return PreRelease(pre_release_type=next_type, number=self.number + 1)

        return PreRelease(pre_release_type=self.pre_release_type, number=self.number + 1)


def guess_pre_release_type(value: str, *, project_type: ProjectTypeEnum) -> PreReleaseTypeEnum:
    maybe_type_mapping = PRE_RELEASE_TYPE_MAPPING.get(project_type)
    if maybe_type_mapping:
        return {item: pre_release_type for pre_release_type, item in maybe_type_mapping.items()}[
            value
        ]
